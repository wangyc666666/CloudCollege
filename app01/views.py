...
import qrcode
from CloudCollege import settings
...
def login(request):
    ...
    :return:
    '''
    ret = {'status': 0, 'message': '', 'category': ''}
    loginSession = request.session.get('is_login', None)
    category = models.Category.objects.all()
    ret['category'] = category
    if_username = models.Admin.objects.filter(username__username=loginSession)
    if not request.session.get('http_referer'):
        request.session['http_referer'] = request.environ.get('HTTP_REFERER')
            ...
            if request.method == 'POST':
                ...
                if user is not None:
                    if adminuser.user_valid == True:
                        ...
                        return redirect(request.session.get('http_referer'), '/')
                    else:
                        ret['message'] = '账户未激活'
                else:
                    user = None
...
def register(request):
    ...
    ret = {'status': 0, 'data': '', 'message': '', 'mailstatus': '', 'category': ''}
    username = request.POST.get('username')
    password = request.POST.get('password')
    ...
    if request.method == 'POST':
        try:
            ...
            message1 = "\n".join([u'{0},欢迎加入cloud云学院'.format(username), u'请访问该链接，完成用户验证:',
...
def category(request, **kwargs):
    ...
    ret = {'message': '', 'clickCount': '', 'document_data': '', 'status': 0, 'data': '', 'count': '', 'nowCount': '',
           ...
           'country': '', 'doc_page': '', 'detail_url': '', 'video_selected': '', 'carousel_data':'', 'advertising_data':''}
    ...
    try:
        ...
        if data_type_int == 0:
            ...
            if loginSession:
                ...
                tags_exist = models.NewType.objects.filter(display=tags)
                if not tags_exist:
                    models.NewType.objects.create(display=tags)
                tags = models.NewType.objects.filter(display=tags)[0]
        ...
        if data_type_int == 1:
            ...
            if loginSession:
                ...
                cover_image_webp_static = chage_foot_image(cover_image_static, 'webp')
                if not lesson_exist:
                    create_video_lesson = models.DocumentData.objects.create(
                        category=category,
                        title=title,
                        content=content,
                        course_price=course_price,
...
def doc_collection(request):
    ...
    if new.focususer.filter(username=username_logined).first():
        ret['message'] = 'already focus'
        return HttpResponse(json.dumps(ret))
    count = new.focususer.count()
    temp = count + 1
    new.focususer.count = temp
    new.save()
    ret['data'] = temp
    new.focususer.add(username_logined)
    ret['message'] = 'ok'
...
def mobile_index(request,**kwargs):
    ...
    ret = {'clickCount': '', 'document_data': '', 'status': 0, 'data': '', 'count': '', 'nowCount': '', 'page': '',
           'username': '', 'category': '', 'nowuser': '', 'city': '', 'province': '', 'country': '', 'area': '',
           'message': '', 'detail_url': '', 'video_selected': '', 'category_url':'','carousel_data':''}
...
def mobile_docindex(request,**kwargs):
    ...
    ret = {'clickCount': '', 'document_data': '', 'status': 0, 'data': '', 'count': '', 'nowCount': '', 'page': '',
           'username': '', 'category': '', 'nowuser': '', 'city': '', 'province': '', 'country': '', 'area': '',
           'message': '', 'detail_url': '', 'video_selected': '','carousel_data':''}
...
def auth_token(request):
    data={ 'code': ''}
    jwt_token=Auth
    auth_token = request.GET.get('token')  or request.META.get('HTTP_AUTH_TOKEN')
...
def wx_auth(request):
    APP_ID = 'wx0a989960039114da'
    APP_SECRET = 'd5b921e909348c181a1b495ebf7bdd29'
    ...
    data=get_token(request, openid).content.decode('utf-8')
   ...
   # uid = user_uuid
    total = models.Admin.objects.filter(username__username=openid).count()
    if total > 0:
        data={'token':key,'code':0,'msg':'已注册'}
        return HttpResponse(json.dumps(data))

    data={'token':key,'code':1000}
...
def mobile_user_center(request):
    ret = {'username':'', 'message':'', 'category':'', 'collect_count':''}
    ...
    show_collect_videos = show_collect.news_set.all()

    show_collect_docs = show_collect.documentdata_set.all()

    collect_count = show_collect_videos.count()+show_collect_docs.count()
    ret['collect_count'] = collect_count
